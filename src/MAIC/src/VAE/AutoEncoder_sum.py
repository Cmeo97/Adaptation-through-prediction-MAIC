...
class AutoEncoder(nn.Module):
    ...
    def perception(self, z1, Im_std, Im_attr, mu_std, mu_attr):
       ...
       Out_mu.backward(mu_std*(mu_attr - Out_mu), retain_graph=True)
       grad_mu = torch.clone(z1.grad)
       # Backward pass for both Image and joints 
       z1.grad = torch.zeros(z1.size(), device=device, dtype=torch.float, requires_grad=True)
       Out_im.backward(Im_std*(Im_attr - Out_im)/0.0135, retain_graph=True)
    ...
    def get_dictionaries(self):
       ...
       for i in range(10000):
           validation_list.append(str(i+40000))
    ...
    def train_net(self, net, network_id, max_epochs , batch_size):
        ...
        STD_max = np.max(self.Im_STD)
        self.Im_STD = torch.tensor(np.float32(self.Im_STD / STD_max), device=device, dtype=torch.float,
                                   requires_grad=True)
        ...
        for epoch in range(max_epochs):
            ...
            for local_batch, local_labels in training_generator:
                ...
                loss, MSE_Im, MSE_z1, output_x = AutoEncoder.run_batch(local_batch, local_labels, True, net, optimizer, criterion, device, self.Im_STD)
            ...
            if epoch % 10 == 0 and epoch<max_epochs-1:
                ...
                for local_batch, local_labels in self.validation_generator:
                    net.eval()
                    loss, MSE_Im, MSE_mu, output_x = AutoEncoder.run_batch(local_batch, local_labels, False, net, optimizer, criterion, device, self.Im_STD)
    ...
    def run_batch(x, y, train, net, optimizer, criterion, device, precision_y):
        ...
        MSE_Im = criterion(output_y + precision_y * output_y, target_y)
        MSE_mu = criterion(q, output_x)
        MSE_z1 = criterion(target, q)
        loss = MSE_Im + MSE_mu + MSE_z1
    ...
    def load_from_file(self, model_id):
        ...
        self.load_state_dict(
            torch.load(os.path.join(self.SAVE_PATH, model_id + "/trained_network" + model_id), map_location='cuda:0'))
        self.eval()
